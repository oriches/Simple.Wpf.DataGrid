namespace Simple.Wpf.DataGrid.Tests.Services
{
    ...
    public sealed class ColumnsServiceFixtures : BaseServiceFixtures
    {
        ...
        public void hides_columns_for_identifier()
        {
            ...
            Assert.That(settings.Get<IEnumerable<string>>(Constants.UI.Settings.Names.Columns).SequenceEqual(columns), Is.True);
            Assert.That(settings.Get<IEnumerable<string>>(Constants.UI.Settings.Names.VisibleColumns).SequenceEqual(columns), Is.False);
            Assert.That(settings.Get<IEnumerable<string>>(Constants.UI.Settings.Names.VisibleColumns).SequenceEqual(new [] { "Col3", "Col4" }), Is.True);
        }
        ...
        public void does_not_hides_columns_for_invalid_identifier()
        {
           ...
           _settingsService.Setup(x => x.TryGet(It.Is<string>(y => y == identifier1), out settings1))
            ...
            Assert.That(settings1.Get<IEnumerable<string>>(Constants.UI.Settings.Names.VisibleColumns).SequenceEqual(columns), Is.True);
        }
        ...
        public void shows_columns_for_identifier()
        {
            ...
            Assert.That(settings.Get<IEnumerable<string>>(Constants.UI.Settings.Names.Columns).SequenceEqual(columns), Is.True);
            Assert.That(settings.Get<IEnumerable<string>>(Constants.UI.Settings.Names.VisibleColumns).SequenceEqual(columns), Is.False);
            Assert.That(settings.Get<IEnumerable<string>>(Constants.UI.Settings.Names.VisibleColumns).SequenceEqual(new[] { "Col1", "Col2" }), Is.True);
        }
